...
- `frontend/src/review/` is the pre-export review step. [reviewItems.ts](frontend/src/review/reviewItems.ts) holds all the logic and no DOM, which is what makes it testable; the components stay thin. Two sources feed the same carousel at two moments: hand-drawn rectangles are known client-side at click time, opaque regions and unreadable fonts only come back in a 409. **Rectangle confirmations never reach the server** — it cannot tell a real confirmation from a claimed one, since the rectangle *is* the instruction, so sending one would fake a guarantee. `serverReviewItems` accepts the body both wrapped in FastAPI's `detail` and bare: it first read only the top level, the carousel never opened, and the unit test missed it because it was fed the shape I had imagined. A run through a real browser found it. One confirm button per item, never a global one.
